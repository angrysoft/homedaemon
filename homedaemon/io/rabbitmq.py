...
class Input(BaseInput):
    def __init__(self, bus: Bus, config: Dict[str, Any], loop):
        ...
        context.load_cert_chain(
            certfile=self.config["rabbitmq"]["certfile"],
            ...
            if not self.connected:
                try:
                    self.connection: amqpstorm.Connection = amqpstorm.Connection(
                        hostname=self.config["rabbitmq"]["host"],
                    ...
                    self.channel.queue.bind(
                        queue="homed_queue",
                        exchange="homedaemon",
                        routing_key=f"homedaemon.{self.config['homed']['homeid']}.in",
                    )
                    self.channel.basic.consume(
                        queue="homed_queue", callback=self.on_message, no_ack=True
                    )
                    self.connected = True
                    self.publish_msg(
                ...
                self.publish_msg(
                    {"cmd": "ping", "homeid": self.config["homed"]["homeid"]},
                    routing_key="homedaemon.main",
                )
    ...
    def publish_msg(self, msg_data: Any, routing_key: str = "") -> None:
        ...
        if self.connected:
            properties = {"content_type": "text/plain", "expiration": "3600"}
            ...
            message: amqpstorm.Message = amqpstorm.Message.create(
                self.channel, txt_msg, properties
            )
