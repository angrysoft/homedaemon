...
class DevicesManager:
    ...
    def register_devices(self) -> None:
        ...
        while self._devices_info_list:
            ...
            self.logger.debug(
                f"Loading..{len(self._devices_info_list)}..{sid} : {device_info.get('name', {}).get('en')} from {device_info.get('place', {}).get('en')}"
            )
            try:
        ...
        while self._scenes_info_list:
            ...
            self.logger.debug(
                f"Loading..{len(self._scenes_info_list)}..{sid} : {device_info.get('name', {}).get('en')} from {device_info.get('place', {}).get('en')}"
            )
            self.register_scene(sid, device_info)

        self.logger.debug(
            f"Devices loaded, devices offline: {self._devices_offline.keys()}"
        )
